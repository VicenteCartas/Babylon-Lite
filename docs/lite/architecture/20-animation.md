...
Per-channel weights are tracked independently per node, because a clip may animate only some of a node's translation / rotation / scale channels. After accumulating all contributions, any channel whose total weight is below 1 is topped up with the node's rest value for the missing weight: rotation slerps rest → accumulated by its weight, translation and scale add the rest value scaled by `1 - weight`. Without this rest-fill, a node animated by only one of two equally-weighted blended clips on a channel would compose from half its value (e.g. half scale) and visibly collapse for the duration of the blend.

The mixer then writes the blended local TRS back to each animated, non-excluded node that has a scene-node target — the same writeback the solo controller performs each tick. Skinning reads the uploaded bone texture, so this does not affect the skinned mesh; it exists so a non-skinned mesh parented to a bone (a static helmet, cape, prop) follows the blended pose during a cross-fade instead of freezing at the fade-start pose until the blend ends. The mixer carries the node targets and excluded-node set in its `[clip, nodes, skeletons, nodeTargets, excludedNodeIndices]` tuple.

Per figure, the mixer keeps one persistent blend target (keyed by the parsed node array) holding the composed TRS, local, and world matrices. Each update first resets every existing target to its rest pose, then accumulates the weighted clip contributions on top, so nodes with no animation channel keep their rest transform. A target is created lazily on a figure's first blended update — after that per-frame reset has already run — so its rest pose is seeded at creation; otherwise an unanimated node (e.g. the skin/armature root) would compose from a zeroed TRS for exactly one frame and collapse the skinned mesh, a one-frame flash on the first cross-fade.
