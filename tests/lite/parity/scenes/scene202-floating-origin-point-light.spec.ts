/**
 * Scene 202 — Floating-Origin Point Light Parity Test
 *
 * Renders a point-lit scene at world (~5e6, *, ~5e6) with Lite's
 ...
 * bake the active camera position into the point-light position so the GPU
...
const REFERENCE_DIR = path.resolve(__dirname, "../../../../reference/lite/scene202-floating-origin-point-light");
const GOLDEN_REF = path.join(REFERENCE_DIR, "babylon-ref-golden.png");

test.skip(!!sceneConfig.skipParity, "Scene 202 skipped via skipParity in scene-config.json");

test("Scene 202 — Floating-origin point light matches Babylon.js reference", async ({ page }, testInfo) => {
    const browser = page.context().browser()!;
    await captureGolden(browser, { sceneId: 202 });
    ...
    expect(useFO, "Scene 202 must report useFloatingOrigin=true on the canvas dataset").toBe("true");
