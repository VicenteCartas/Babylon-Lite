...
const REFERENCE_DIR = path.resolve(__dirname, "../../../../reference/lite/scene157-gltf-weighted-blend");
const GOLDEN_REF = path.join(REFERENCE_DIR, "babylon-ref-golden.png");
const SEEK_TIME = 1.2;

test.skip(!!sceneConfig.skipParity, "Scene 157 skipped via skipParity in scene-config.json");

test("Scene 157 — glTF weighted animation blend matches Babylon.js reference", async ({ page }, testInfo) => {
    test.setTimeout(120_000);

    const browser = page.context().browser()!;
    await captureGolden(browser, { sceneId: 157, seekTime: SEEK_TIME, timeout: 120_000, settleMs: 500 });

    await page.goto(`/scene157.html?seekTime=${SEEK_TIME}`);
