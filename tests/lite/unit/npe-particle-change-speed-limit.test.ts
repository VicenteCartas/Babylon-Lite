...
/**
 * CPU determinism test for the "Change - Speed Limit" NPE graph (playground #ZAE8XN#0): a ParticleSystem
 * with a velocity-limit gradient — addLimitVelocityGradient(0, 10) + (1, 0.1), limitVelocityDamping 0.1,
 * emit power 8 — converted to NPE. Each frame the particle's speed (velocity magnitude) is compared against
 * the per-age limit, and its velocity is braked when it exceeds the limit. This is the first Change test to
 * need NEW blocks: ParticleVectorLengthBlock (speed = |velocity|) and ParticleConditionBlock (brake when
 * speed > limit) feeding UpdateDirectionBlock. Asserts every particle's state matches the committed Babylon.js
 * ground truth.
 */
