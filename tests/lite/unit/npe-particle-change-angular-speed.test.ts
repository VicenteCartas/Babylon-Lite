...
import graphSource from "./fixtures/change-angular-speed-npe.json";
import groundTruth from "./fixtures/change-angular-speed-states.json";
...
/**
 * CPU determinism test for the "Change - Angular Speed" NPE graph (playground #NBQ82W#0): a ParticleSystem
 * with two angular-speed gradients — addAngularSpeedGradient(0, 0) and addAngularSpeedGradient(1, 12) — plus
 * scaleX 0.5, converted to NPE. The per-frame angular speed is a scalar ParticleGradientBlock evaluated at
 * the age/lifetime ratio that feeds UpdateAngleBlock, so a particle's rotation accumulates faster over its
 * life (reusing existing blocks, like Change-Speed but on angle instead of position). Asserts every particle's
 * state matches the committed Babylon.js ground truth.
 */
describe("NPE particle simulation (Change - Angular Speed) — deterministic parity with Babylon.js", () => {
