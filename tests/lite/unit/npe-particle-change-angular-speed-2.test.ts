...
import graphSource from "./fixtures/change-angular-speed-2-npe.json";
import groundTruth from "./fixtures/change-angular-speed-2-states.json";
...
/**
 * CPU determinism test for the "Change - Angular Speed 2" NPE graph (playground #Y37E4A#0): a ParticleSystem
 * with a MULTI-STOP parabolic angular-speed gradient — addAngularSpeedGradient over 40 stops of
 * 30 * (1 - (i - 1)^2), rising 0 -> 30 -> 0 across a 5s lifetime — plus scaleX 0.5, converted to NPE. Unlike
 * the 2-stop Change tests, this exercises the ParticleGradientBlock's multi-segment interpolation (correct
 * stop selection + lerp across 40 stops) feeding UpdateAngleBlock, still with zero new blocks. Asserts every
 * particle's state matches the committed Babylon.js ground truth.
 */
describe("NPE particle simulation (Change - Angular Speed 2, multi-stop) — deterministic parity with Babylon.js", () => {
