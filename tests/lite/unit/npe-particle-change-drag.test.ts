...
import graphSource from "./fixtures/change-drag-npe.json";
import groundTruth from "./fixtures/change-drag-states.json";
...
/**
 * CPU determinism test for the "Change - Drag" NPE graph (playground #H5J2I5#0): a ParticleSystem with two
 * drag gradients — addDragGradient(0, 0.1) and addDragGradient(1, 1) — over a 4s lifetime, converted to NPE.
 * The per-frame drag factor is a scalar ParticleGradientBlock evaluated at the age/lifetime ratio that scales
 * down the velocity feeding UpdatePositionBlock, so a particle decelerates as its drag rises toward 1 (reusing
 * existing blocks, like Change-Speed). Asserts every particle's state matches the committed Babylon.js ground
 * truth.
 */
describe("NPE particle simulation (Change - Drag) — deterministic parity with Babylon.js", () => {
