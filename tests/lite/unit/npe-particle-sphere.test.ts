...
/**
 * CPU determinism test for the Node Particle (NPE) runtime with a sphere emitter. Builds the same graph the
 * Scene 263 lab scene uses, seeds Math.random exactly like the Babylon.js oracle, steps the simulation, and
 * asserts every particle's state matches the committed Babylon.js ground truth — the fast (no GPU) guard for
 * the SphereShapeBlock's per-particle Math.random consumption and radial-direction math. The pixel parity
 * test (scene263-npe-sphere.spec.ts) covers the rendering side.
 */
