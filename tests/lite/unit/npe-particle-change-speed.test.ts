...
import graphSource from "./fixtures/change-speed-npe.json";
import groundTruth from "./fixtures/change-speed-states.json";
...
/**
 * CPU determinism test for the "Change - Speed" NPE graph (playground #WJPDMN#0): a ParticleSystem with two
 * velocity gradients — addVelocityGradient(0, 0.5) and addVelocityGradient(1, 10) — converted to NPE. The
 * per-frame speed factor is a scalar ParticleGradientBlock evaluated at the age/lifetime ratio that scales
 * the emission direction feeding UpdatePositionBlock, so a particle accelerates from 0.5 to 10 over its life
 * (proving the gradient machinery also drives velocity integration, reusing existing blocks). Asserts every
 * particle's state matches the committed Babylon.js ground truth.
 */
describe("NPE particle simulation (Change - Speed) — deterministic parity with Babylon.js", () => {
