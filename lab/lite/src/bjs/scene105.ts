...
// (port of PG #WO0H1U#166, BJS reference). scene104 PLUS a kinematic moving platform and camera (5,5,-5).
...
const AUTOTEST_INPUT = new Vector3(0.57, -0.5, 2);
...
// Moving ANIMATED platform (PG #WO0H1U#166).
const PLATFORM_POS = new Vector3(-4, 0, -12);
const PLATFORM_ROT_PER_STEP = 0.005;
const PLATFORM_TIME_PER_STEP = 1 / PHYSICS_FPS;

// Dynamic obstacle boxes placed in the auto-test path so onTriggerCollisionObservable fires.
const OBSTACLES = [new Vector3(-1.02, -0.1, -11.41), new Vector3(-1.54, -0.1, -10.45), new Vector3(-2.1, -0.1, -11.4)];
...
(async function () {
    ...
    OBSTACLES.forEach((p, i) => {
        const box = MeshBuilder.CreateBox("obstacle" + i, { size: 1 }, scene);
        box.position.copyFrom(p);
        box.material = makeMaterial(CUBE_COLOR);
        new PhysicsAggregate(box, PhysicsShapeType.BOX, { mass: boxMass }, scene);
    });

    // Moving ANIMATED platform (PG #WO0H1U#166). Procedural box in WORLD coords. Kinematic body with
    // disablePreStep=false so setting the mesh transform each physics step drives the body.
    const platform = MeshBuilder.CreateBox("movingPlatform", { width: 4, height: 0.2, depth: 4 }, scene);
    platform.position.copyFrom(PLATFORM_POS);
    platform.rotationQuaternion = new Quaternion(0, 0, 0, 1);
    platform.material = makeMaterial(CUBE_COLOR);
    const platformAgg = new PhysicsAggregate(platform, PhysicsShapeType.BOX, { mass: 0 }, scene);
    ...
    scene.onAfterPhysicsObservable.add(() => {
        ...
        platform.rotationQuaternion = new Quaternion(0, Math.sin(half), 0, Math.cos(half));
