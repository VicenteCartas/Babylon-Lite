// Scene 263: Node Particle Editor — "Emitters - Sphere".
//
// Builds a Node Particle graph (a classic ParticleSystem with a sphere emitter, converted to NPE) and renders
// its particles as camera-facing additive billboards. The simulation is stepped a fixed number of times with the
// RNG seeded deterministically, so the frame is frozen and reproducible for pixel parity against the Babylon.js oracle.
...
async function main(): Promise<void> {
    ...
    const camera = createArcRotateCamera(-Math.PI / 2, 1.2, 14, { x: 0, y: 0, z: 0 });
